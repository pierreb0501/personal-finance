...
- **Actual entries** — `getTransactionsForMonth(db, year, month)`, grouped by `date`. `getTransactionsForMonth` already filters `pending = 0` but does **not** filter `ignored` — `getCalendarMonth` must additionally filter out `ignored = 1` rows itself before computing `netTotal` and building the day's actual-entry list (matching how `/spending`'s totals exclude ignored transactions elsewhere, e.g. `getMonthlySpend`). A day's `netTotal` is the sum of `-amount` for that day's remaining (non-pending, non-ignored) transactions — Plaid convention: positive `amount` = spend, negative = income/credit, so negating gives "income minus spend".
- **Expected entries** — from `getCommittedItemsWithStatus(db, year, month)` (uses `expectedDay`) and `getRecurringMerchantsWithStatus(db, year, month)` (uses `dayOfMonth`), filtered to items where `confirmedAmount === null` (hasn't posted yet this month) **and** the projected day is `>= today` (so past months and already-elapsed days in the current month never show stale forecasts). Projected day is clamped to the month's last real day (e.g. day 31 in a 30-day month → day 30). `likelyCancelled` recurring merchants are excluded from forecasts. Partial confirmation (e.g. a committed item matching more than one transaction, or matching a smaller amount than expected) is treated as fully confirmed — once `confirmedAmount` is non-null at all, the expected entry is dropped; no "remaining gap" forecast is shown.
...
- `components/NavLinks.tsx` — current order is Overview, Net Worth, Spending, Recurring, Budget, Investments, Categories, Accounts. Add `{ label: 'Calendar', href: '/calendar', icon: Calendar }` (lucide-react) directly after "Spending" and before "Recurring" — it's a daily drill-down on spending data, and sits next to "Recurring" since both surface forward-looking planning info.
...
- Unit tests in `__tests__/` (new `calendar.test.ts`, following `db.test.ts`'s conventions) covering `getCalendarMonth`: ignored-transaction exclusion, actual/expected merge, the "stops being expected once confirmed" dedup, day-of-month clamping for short months, and the `>= today` cutoff for forecasts. "Today" uses server-local time (`new Date()`), consistent with the existing `getRecurringMerchants`/`getRecurringMerchantsWithStatus` usage elsewhere in `queries.ts`.
